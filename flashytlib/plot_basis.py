def plt_Eos_DTY( r, D, T, Y, optional_title, nfile = 1 ):
    ...
    if (nfile > 1):
        ...
        for ifile in range(nfile):
            ax1.plot(r[ifile], T[ifile],\
                     ...
                     linewidth=LineWeights[ifile%len(LineWeights)], \
                     linestyle=LineTypes[ifile%len(LineTypes)],\
                     label='{:d} '.format(ifile)+'Temperature')
        # --- electron fraction
        ...
        for ifile in range(nfile):
            ax2.plot(r[ifile], Y[ifile], \
                     ...
                     linewidth=LineWeights[ifile%len(LineWeights)], \
                     linestyle=LineTypes[ifile%len(LineTypes)],\
                     label='{:d} '.format(ifile)+'Electron Fraction')
        ax2.tick_params(axis='y', labelcolor=color)
        fig.tight_layout()  # otherwise the right y-label is slightly clipped
        lines, labels = ax1.get_legend_handles_labels()
        lines2, labels2 = ax2.get_legend_handles_labels()
        ax1.legend(lines + lines2, labels + labels2,\
                   loc='upper center', bbox_to_anchor=(0.4, 0.9),
           ncol=3 )
